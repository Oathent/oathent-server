...
function hasDuplicates(scopes: Scopes) {
    return (
        new Set(Object.keys(scopes)).size !== Object.keys(scopes).length &&
...
function hasLessThan256(scopes: Scopes) {
    return Object.values(scopes).reduce((p, c) => p || c < 256, false);
}

function hasCustomOverwrittenDefaultScopes() {
    return (
        Object.keys(SCOPES).length !=
        Object.keys(DEFAULT_SCOPES).length + Object.keys(CUSTOM_SCOPES).length
    );
}
...
export async function initialiseScopes() {
    if (hasNonPowers(DEFAULT_SCOPES))
        throw new Error(
            'Default scopes contained values that were not powers of 2! This should never happen!',
        );

    if (hasDuplicates(DEFAULT_SCOPES))
        throw new Error(
            'Default scopes contained duplicated values! This should never happen!',
        );

    try {
        await fs.access(CUSTOM_SCOPES_FILE, fs.constants.R_OK);
        CUSTOM_SCOPES = JSON.parse(
            await fs.readFile(CUSTOM_SCOPES_FILE, 'utf-8'),
        );
    } catch (e) {}

    if (hasLessThan256(CUSTOM_SCOPES))
        throw new Error(
            'Custom scopes contained values that were less than 512! Please fix your configuration',
        );

    if (hasNonPowers(CUSTOM_SCOPES))
        throw new Error(
            'Custom scopes contained values that were not powers of 2! Please fix your configuration',
        );

    if (hasDuplicates(CUSTOM_SCOPES))
        throw new Error(
            'Custom scopes contained duplicated values! Please fix your configuration',
        );
    ...
    if (hasDuplicates(SCOPES) || hasCustomOverwrittenDefaultScopes())
        throw new Error(
            'Custom scopes overwrote 1 or more default scopes! Please fix your configuration!',
        );
}
