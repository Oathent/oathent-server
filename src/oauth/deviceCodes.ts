...
export function addDeviceCodeInfo(deviceCode: string, status: DeviceCodeStatus, userId: bigint, appId: bigint, scope: number, expiry: number) {
...
export function setDeviceCodeStatus(deviceCode: string, status: DeviceCodeStatus) {
    if (deviceCodes[deviceCode])
        deviceCodes[deviceCode].status = status;
}
